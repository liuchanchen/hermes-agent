...
# ⚠️ 不要用 Python 循环逐个 git add（873+ skill 文件时超时 30s+）。
...
git add .hermes_backup/skills/  # 设置 timeout≥120s；873 文件实测 2-5s
...
- state.gz.part_* 分卷文件: 不存在，无需清理
- state.db / state.db.gz 已确认在 `.gitignore` 中
