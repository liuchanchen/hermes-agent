---
name: remote-70-96
description: 远程操作 GPU 服务器 (10.10.70.96 / oem96) — SSH 连接、CUDA Toolkit 安装、NCCL 管理、8×RTX 5090 环境搭建
version: 1.0.0
author: Hermes Agent
license: MIT
metadata:
  hermes:
    ...
    related_skills: [remote-70-93, remote-70-95, remote-70-88]
---

# Remote GPU Server (10.10.70.96 / oem96)

远程操作 GPU 服务器 10.10.70.96 (oem96) 的常用工作流。与 10.10.70.93 (oem93) 和 10.10.70.95 (oem95) 完全相同的硬件配置，
三台服务器构成 GPU 集群，通过 Mellanox ConnectX-6 Dx NIC 互联。

> **与 93/95 的差异：** 70.96 内核为 **5.15.0-43-generic**（同 93），磁盘使用 LVM（同 95）。
> 其余配置完全一致。
...
| sudo | 需要密码（同 70.88/93/95） |

### GPU 拓扑

与 70.93/95 完全一致：8 张 GPU 分布在 2 个 NUMA 节点。
...
- **CUDA**: 13.2 nvcc + 完整 CUDA dev 库
- **完成时间**: 2026-05-10 00:30 CST
...
```bash
# 首次连接接受 host key
ssh -o StrictHostKeyChecking=accept-new jianliu@10.10.70.96 "echo connected"

# 交互式登录（自动加载 ~/.bashrc 环境）
ssh -t jianliu@10.10.70.96 "source ~/.bashrc && bash -l"

# 执行命令并返回（务必 source ~/.bashrc）
ssh jianliu@10.10.70.96 "source ~/.bashrc && command"

# 需要 sudo 时通过管道传密码（同时 source ~/.bashrc）
ssh jianliu@10.10.70.96 "source ~/.bashrc && echo 'PASSWORD' | sudo -S apt-get install -y PACKAGE"
```
...
- 与 70.93、70.95 **完全同配置**
- 三台服务器通过 Mellanox ConnectX-6 Dx 组网，可用于多机 NCCL 测试（参考 remote-70-93 skill 的多机 NCCL 测试部分）
