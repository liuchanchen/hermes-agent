...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # Match hermes_cli.config.get_env_value precedence: live process
    # environment wins over ~/.hermes/.env, then falls back to the file.
    ...
    if provider == "openrouter":
        token = _get_env_value("OPENROUTER_API_KEY")
        if token:
